...
const OmniChannelServiceStandalone = (props: OmniChannelServiceStandaloneProps) => {
  ...
  // single socket ref for this component (will not recreate on each render)
  const socketRef = useRef<Socket | null>(null);

  // create socket when user is available. do not autoConnect
  useEffect(() => {
    if (!user?.agent_detail?.uuid) return;

    // if already created and same endpoint, reuse
    ...
    s.on("connect", () => {
      console.info("[omni-socket] connected", s.id);
    });
    s.on("disconnect", (reason) => {
      console.info("[omni-socket] disconnected:", reason);
    });
    s.on("connect_error", (err) => {
      console.warn("[omni-socket] connect_error:", err?.message || err);
    });
    ...
    // only run when user.agent_detail.uuid changes
  ...
  // helper: connect if needed and wait for connection (or timeout)
  const ensureConnected = async (timeoutMs = 3000): Promise<boolean> => {
    const s = socketRef.current;
    if (!s) return false;

    if (s.connected) return true;

    // If already in the process of connecting, wait for the connect event or timeout
    // if (s.connecting) {
    ...
    // otherwise call connect and wait
    try {
      s.connect();
    } catch (err) {
      console.warn("[omni-socket] connect() threw:", err);
    }
  ...
  const handleAccept = async () => {
    ...
    // now emit via socket, but ensure socket exists and connected
    const s = socketRef.current;
    if (!s) {
      // no socket instance created — nothing to emit to
      console.warn("[omni-socket] no socket instance, skipping emit");
      return;
    }

    const connected = await ensureConnected(3000);
    if (!connected) {
      console.warn("[omni-socket] socket did not connect in time — emit skipped or queued if you implement queueing");
      // Optionally: you can queue the accept action to attempt later
      return;
    }

    // emit accept event (safe)
    try {
      s.emit("om:accept", {
